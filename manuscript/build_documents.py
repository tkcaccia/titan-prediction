...
doc.add_paragraph("Table 2 records the three fitted UCEC artifacts locked for any future untouched evaluation. The displayed performance values are internally derived TCGA estimates included only to document why the targets were selected; they are not external benchmarks. All three must be reported under the protocol, including chance-level performance, extraction failure or endpoint non-evaluability.")
add_table(
    doc,
