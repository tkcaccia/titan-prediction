...
if highlighted:
    ...
    add_table(doc, ["Cancer–endpoint", "Type", "Metric 1", "Metric 2", "Metric 3"], performance_rows)
...
doc.add_heading("Liver and pancreatic cancer examples", level=2)
doc.add_paragraph("LIHC yielded " + cancer_result_text("LIHC") + ". PAAD yielded " + cancer_result_text("PAAD") + ". These are discovery results; the global-correction status of every candidate is reported in the machine-readable atlas.")
...
doc.add_heading("Practical contribution of the PLS framework", level=2)
doc.add_paragraph("The contribution of TITAN plus PLS is a uniform, auditable screen over many prespecified outcomes using one fixed representation, with one-at-a-time PLS–LDA retained for the prespecified individual binary endpoints and PLS2 examined secondarily for correlated inflammatory panels. This study was not designed to compare PLS with elastic-net, convolutional or other machine-learning approaches, and no claim of algorithmic superiority is made.")
...
responses = [
    ...
    ("Additional change: rSVD-only PLS decomposition",
     "All primary, permutation, repeated, site-grouped, slide-pooling, PLS1–PLS2 and final-model fits now explicitly use CPU rSVD with 10 oversampling vectors, two power iterations and fixed seeds. IRLBA is not used. Solver identity and controls are recorded in screening, sensitivity and model-registry metadata and verified against saved-model diagnostics."),
