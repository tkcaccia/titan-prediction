...
add_labelled(doc, "Conclusions.", "This systematic patient-level TCGA benchmark defines target-specific predictability under one fixed pretrained TITAN representation and distributes compact fitted research models. Its contribution is methodological and resource-oriented rather than mutation-target novelty; independent external validation remains required before clinical interpretation.")
...
doc.add_paragraph(
    f"The TITANPred R package exposes {len(supported_c) + len(supported_b):,} fitted models "
    f"({len(supported_c):,} continuous and {len(supported_b):,} binary) across "
    f"{len({r['tumor_type'] for r in supported_c + supported_b})} cancers [32]. Users supply "
...
doc.add_paragraph("Using mean-pooled fixed pretrained TITAN representations, this study provides a systematic patient-level TCGA benchmark of selected immune, mutation, pathway, MSI, aneuploidy and fusion phenotypes. The public TITANPred package applies all cancer-matched fitted PLS and PLS–LDA models without distributing patient-level training data. The contribution is a transparent evaluation and reusable research resource—not broad endpoint novelty or clinical validation—and locked independent testing remains essential.")
...
responses = [
    ...
    ("Audit comment: the Introduction did not state the gap",
     "Addressed. The Background now states explicitly that breadth is not the gap after the large pan-cancer studies by Fu, Kather, Saldanha and especially Arslan. It defines the narrower gap as a reproducible patient-level benchmark using one fixed pretrained representation, deterministic slide aggregation, one nested validation framework, complete negative and ineligible reporting, target-level site sensitivity and deployable fitted models."),
    ...
    ("4. Reproducibility materials must be deposited before submission",
     f"Addressed in two public repositories: the analysis repository ({REPO}) contains the exact analysis plan, source URLs/checksums, software commit, eligibility tables, code for every analysis and figure, out-of-fold prediction outputs and literature crosswalk; the TITANPred package repository ({MODEL_REPO}) distributes all 323 fitted models, their SHA-256 registry, inference interface, repeated out-of-fold reference distributions and HTML/PDF report template. An archived versioned release with a persistent DOI remains to be created before submission."),
