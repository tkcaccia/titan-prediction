...
MANUSCRIPT_TITLE = (
    "A patient-level comparison of pathology foundation models for molecular and "
    "derived immune-feature prediction across 32 TCGA cancers"
)
...
add_labelled(doc, "Background.", "Whole-slide foundation models can compress histology into reusable representations, but molecular-prediction results may depend on the selected representation. We compared TITAN, Giga-SSL and Prov-GigaPath under one patient-level design rather than claiming unprecedented mutation targets.")
add_labelled(doc, "Methods.", f"We deterministically mean-pooled primary-tumour diagnostic-slide embeddings before outcome matching and used the patient as the validation unit. The primary TITAN analysis tested {len(continuous):,} continuous and {len(binary):,} binary cancer–endpoint pairs using nested PLS1 or PLS–LDA, complete-process permutation testing and multiplicity control. A locked representation benchmark evaluated all three foundation models on the same {foundation_common_n:,} patients, identical outcome-labelled subsets, folds, seeds and tuning rules across 1,507 continuous and 426 binary tasks.")
add_labelled(doc, "Results.", f"The primary TITAN screen identified {len(supported_c):,} continuous and {len(supported_b):,} binary candidates. On the common cohort, TITAN, Prov-GigaPath and Giga-SSL met the screening threshold for 208, 127 and 105 continuous tasks and 93, 82 and 66 binary tasks, respectively. Target-level effects remained correlated with TITAN (Spearman 0.793–0.805 for continuous and 0.628–0.663 for binary tasks), but the representation ranking varied by endpoint. Grouping by TCGA tissue-source-site code reduced {ival(site_combined.get('below_threshold_models'))}/{ival(site_combined.get('screen_positive_models'))} primary TITAN candidates ({fnum(site_combined.get('below_threshold_percent'), 1)}%) below threshold.")
add_labelled(doc, "Conclusions.", "Foundation-model choice materially changes which molecular and derived immune-feature associations cross an internal screening threshold. TITAN had the broadest coverage in this matched TCGA benchmark, but no representation was uniformly best and none has independent external validation. The accompanying multi-representation R package is a research interface for locked future evaluation, not a clinical prediction system.")
...
doc.add_paragraph("The narrower unresolved gap is a reproducible, endpoint-matched comparison of multiple pathology foundation models that deterministically aggregates slides before outcome matching; performs nested validation with the patient as the unit; preserves tested-negative and sample-size-ineligible results; reports sensitivity to grouping by TCGA tissue-source-site code; models selected derived immune and genomic-context features continuously; and constructs compact fitted predictors with research-use provenance metadata. These design and resource features—not unprecedented endpoint classes—define the contribution of this study.")
doc.add_paragraph("We evaluated three complementary pretrained representations. TITAN is a multimodal whole-slide model trained with visual self-supervision and vision–language alignment; its published Mass-340K pretraining corpus excluded TCGA, although TCGA was used for downstream evaluation [16]. Giga-SSL is a gigapixel self-supervised whole-slide representation whose official repository distributes 512-dimensional TCGA embeddings [39]. Prov-GigaPath combines a tile encoder with a long-context slide encoder trained on Providence health-system pathology data; we used the final 768-dimensional slide layer from the public TCGA embedding dataset [40]. No weights were fine-tuned. These sources differ in pretraining data, objectives and released TCGA coverage, so the matched comparison estimates representation-associated differences under this pipeline rather than intrinsic superiority in all settings.")
...
doc.add_paragraph("The primary question was cancer-specific: which individual mutations, derived inflammatory features, oncogenic pathways, MSI phenotypes, aneuploidy measures and fusions show held-out cross-validated statistical association with a foundation-model representation within each cancer, and how stable is that conclusion across TITAN, Giga-SSL and Prov-GigaPath? Throughout this article, ‘predictability’ is shorthand only for that operational cross-validated association at the reported performance threshold. It does not denote causal biological information, a morphological mechanism, direct recovery of the source measurement or justification for replacing an assay. Molecular subtype association was not analysed.")
add_portrait_section(doc)

doc.add_heading("Methods", level=1)
doc.add_heading("Study design, slides and patient unit", level=2)
doc.add_paragraph(f"The published TITAN TCGA table contained {n_slides:,} eligible primary-tumour diagnostic slides (TCGA sample type 01 and –DX filename) from {n_patients:,} participants. The {n_multi:,} participants with multiple eligible slides contributed one patient vector obtained by feature-wise arithmetic mean before outcomes were joined. Patient—not slide—was the independent cross-validation unit. Within a patient, every eligible slide received weight 1/n; after pooling, every participant contributed one row and therefore one observational unit. TITAN's TCGA-Slide-Reports.csv matched {n_exact_reports:,} selected slides exactly by filename; {n_unmatched_reports:,} slides lacked an exact report row. Report metadata were used only to audit identifiers, project/cancer provenance and anatomical resection/biopsy-site annotations, and no report text entered a model. The two-character tissue-source-site code used for grouped validation was derived directly from the TCGA participant barcode. {n_missing_cancer:,} participants without a resolvable cancer label did not enter cancer-specific modelling. No participant had eligible slides from more than one primary sample barcode.")
doc.add_paragraph("No structured tumour-content/cellularity percentage, tissue-area measurement, artefact assessment, biopsy-versus-resection indicator or slide/image-quality score was available in the TITAN feature table or TCGA-Slide-Reports.csv; accordingly, none was used for slide exclusion or weighting. The field named site_of_resection_or_biopsy contains an anatomical site, not specimen procedure. Generated slide narratives were audited only for transparency and were not treated as independent pathologist-adjudicated QC labels. Eligibility therefore reflected TCGA sample type and diagnostic-slide filename, not prospective pathology review.")
...
doc.add_paragraph("Predictors were fixed pretrained whole-slide representations: 768 TITAN dimensions [16], 512 Giga-SSL dimensions [39] and the final 768-dimensional Prov-GigaPath slide layer [40]. The TITAN artifact was obtained from the official gated Hugging Face release; Giga-SSL embeddings were taken from the official GitHub release (commit 605b950); and Prov-GigaPath embeddings were obtained from the public Hugging Face TCGA dataset. Conversion scripts preserve slide identifiers, deterministic feature names and SHA-256 provenance. Exact duplicate Prov-GigaPath slide rows were verified to have identical final-layer vectors and deduplicated deterministically. No slide pixels were downloaded or reprocessed for the added representations and no foundation-model weights were fine-tuned.")
...
doc.add_paragraph(f"For the representation comparison, each model was independently mean-pooled across eligible primary-tumour diagnostic slides. The intersection contained {foundation_common_n:,} patients. Every cancer–endpoint task used the identical outcome-labelled patients, outer folds, inner folds, seeds, component range and rSVD settings for all three representations. The comparison includes all 1,507 continuous and 426 binary tasks meeting eligibility on this common cohort; neither targets nor examples were selected using a representation's performance. Q², RMSE and Spearman correlation were reported for continuous outcomes, and balanced accuracy, AUROC and PR-AUC for binary outcomes. This is an internal TCGA representation benchmark, not external validation.")
...
doc.add_paragraph("To quantify whether TITAN itself retained information about TCGA tissue-source-site code, we treated that code as a multiclass outcome within each cancer. This dedicated confounding analysis retained codes represented by at least 10 patients, required at least two eligible codes per cancer, and used five independently seeded nested five-fold PLS–LDA validations with the same component range and rSVD controls. The primary metric was multiclass macro balanced accuracy (mean recall across TCGA tissue-source-site codes); chance was 1/k for k analysed codes, and a chance-normalised value, (balanced accuracy − 1/k)/(1 − 1/k), was calculated for cross-cancer description. Patients with rare codes excluded from this code-classification analysis remained in all eligible molecular-endpoint analyses. This analysis tests whether the representation encodes signatures associated with TCGA tissue-source-site code; it does not identify their scanner, laboratory, staining or population causes.")
...
doc.add_paragraph("PathoFMPred accepts TITAN, Giga-SSL or Prov-GigaPath features through an explicit foundation_model argument and validates the representation-specific feature schema before patient-level pooling. It retains binary LDA scores but does not convert them to probabilities. Research outputs label their TCGA out-of-fold score rank as not a probability and place training denominators and evidence warnings beside each estimate. Representation availability in the package does not imply external validation or clinical suitability.")
...
doc.add_paragraph("Analyses used R 4.6.0 and fastPLS 0.99.20 (Git commit dcf45cc). Version-controlled code, source manifests, target catalogues, matched out-of-fold predictions, figures and model registries are organized in the public companion analysis repository [30]. The renamed access-controlled GPL-3 R package, PathoFMPred, contains representation-specific fitted objects, feature schemas, SHA-256 registries, reference distributions and a research-software output template [32]. It requires an explicit foundation_model selection. The repository is private and no public availability of fitted artifacts is claimed. No independent cohort with compatible representation and endpoint labels was included; every performance estimate is an internally derived TCGA estimate.")
...
doc.add_paragraph("Supplementary methods, Tables S1–S15, Figures S1–S4 and the machine-readable-file inventory are provided in Additional file 1. The two post hoc illustrative COAD research-software outputs are supplied separately as Additional files 2 and 3.")

doc.add_heading("Results", level=1)
doc.add_heading("Cohort and analysis coverage", level=2)
doc.add_paragraph(f"The embedding cohort contained {n_patients:,} patients represented by {n_slides:,} slides; {n_multi:,} patients ({100*n_multi/n_patients:.1f}%) had more than one eligible diagnostic slide. The maximum was 30 slides for {maximum_slide_patient.get('patient', 'one participant')} ({maximum_slide_patient.get('project_id', 'SARC')}); its 30 slides each received weight 1/30 in the pooled vector, while the resulting patient vector had the same downstream observational weight as every other participant. No patient contributed slides from multiple primary sample barcodes. Exact slide-report coverage was {n_exact_reports:,}/{n_slides:,} ({100*n_exact_reports/n_slides:.1f}%). Cancer labels were available for {n_patients-n_missing_cancer:,} patients across {n_cancers} cancers. The primary screen evaluated {len(continuous):,} continuous and {len(binary):,} binary cancer–endpoint pairs; full family-level counts are in Supplementary Table S1. Of {len(mutation_eligibility):,} prespecified cancer–gene mutation pairs with a profiled denominator, {n_mutation_eligible:,} met the 20-positive/20-negative rule and {n_mutation_ineligible:,} were reported as ineligible rather than tested-negative.")
...
doc.add_heading("Matched comparison of three foundation-model representations", level=2)
doc.add_paragraph(
    ...
    "The comparison therefore removes patient and endpoint composition as explanations for between-representation differences, while remaining conditional on TCGA and each public embedding release."
)
doc.add_paragraph(
    "TITAN met the prespecified screening threshold for 208 continuous and 93 binary tasks, compared with 127 and 82 for Prov-GigaPath and 105 and 66 for Giga-SSL, respectively. "
    ...
    "TITAN had higher target-level effect statistics more often overall, but both alternative representations were superior for substantial subsets. Consequently, the relevant translational result is representation sensitivity at the individual target level, not a universal ranking."
)
add_table(
    doc,
    ["Representation", "Dimension", "Patients", "Continuous ≥ threshold", "Binary ≥ threshold", "External validation"],
    [
        [m, foundation_cohort_by_model[m]["dimensions"], f"{ival(foundation_cohort_by_model[m]['patients']):,}",
         foundation_summary_by_key[(m, "continuous")]["screening_positive"],
         foundation_summary_by_key[(m, "binary")]["screening_positive"], "None"]
        for m in ("TITAN", "GigaSSL", "ProvGigaPath")
    ],
    widths=[3.0, 1.8, 2.0, 3.0, 2.7, 2.5],
)
...
doc.add_paragraph(
    ...
    f"PathoFMPred's TITAN interface now includes {len(binary_standard)} binary and {len(supported_c)} continuous models and omits "
...
add_figure(doc, "Figure5_supported_counts.png", "Figure 5. Breadth and family of screen-positive targets in the 15 cancers with the largest candidate counts. Restricting the display improves label size; complete cancer-level counts are in Supplementary Table S1. Absence of a target is distinct from a tested screen-negative result.")
...
doc.add_heading("Sensitivity to grouping by TCGA tissue-source-site code", level=2)
doc.add_paragraph(
    f"Internal validation grouped by TCGA tissue-source-site code was feasible for {sum(r.get('feasible')=='TRUE' for r in site_c)+sum(r.get('feasible')=='TRUE' for r in site_b)} screen-positive models. "
    ...
    "training and test patients, so the decline cannot be attributed solely to confounding by that code; nevertheless, the near-chance "
...
doc.add_paragraph(
    ...
    + ". ACC, CHOL, DLBC, MESO and UCS had fewer than two codes meeting the 10-patient requirement and were reported as ineligible. This result shows a held-out statistical association between the fixed TITAN representation and TCGA tissue-source-site code and strengthens the confounding concern; it does not identify causal scanner, laboratory or staining information."
)
add_figure(doc, "Figure6_site_grouped_sensitivity.png", "Figure 6. Sensitivity to grouping by TCGA tissue-source-site code as a principal result. Panel A compares all 323 random-fold estimates with estimates grouped by TCGA tissue-source-site code; Panel B shows the six largest attenuations; Panel C shows the 10 strongest chance-normalised within-cancer code-classification results. Larger typography and restricted ranked panels improve readability. This grouping remains internal to TCGA; the code is not an institutional, scanner or laboratory identifier, and the analysis is not external validation.")

doc.add_heading("Multiple-slide sensitivity", level=2)
doc.add_paragraph(
    f"Replacing each deterministic patient mean with the lexicographically first eligible diagnostic slide produced a median first-slide-minus-mean-pool performance change of {fnum(median(pool_deltas))} across screen-positive models. This patient-level sensitivity is distinct from the analysis grouped by TCGA tissue-source-site code."
)
...
doc.add_paragraph("No non-TCGA patient entered model evaluation, and no external performance result is reported. The prespecified targets and protocol for future independent evaluation are provided in Supplementary Table S6c and the software documentation.")

doc.add_heading("Illustrative COAD profiles", level=2)
doc.add_paragraph(
    "Two COAD participants, TCGA-AA-A01F and TCGA-AA-3972, illustrate contrasting continuous and binary profiles (Figure 7). The pair was chosen post hoc after clinical-text and profile-saturation restrictions and then maximising Euclidean separation across continuous TCGA OOF prediction ranks; the procedure intentionally enhances visual contrast. Nodal status was not matched: TCGA-AA-A01F was reported as pN1, whereas the available slide summary for TCGA-AA-3972 did not state a nodal category. The cases are not presented as a matched clinical comparison, and treatment, response, recurrence, follow-up and survival are neither shown nor interpreted. This is an illustration, not performance, prevalence, prognosis or treatment-response evidence."
)
add_figure(
    doc,
    "Figure7_COAD_TITANPred_examples.png",
    "Figure 7. Post hoc PathoFMPred research-software visualization for two COAD participants using TITAN inputs. Panel A uses a common-scale dumbbell comparison of eight continuous TCGA OOF ranks; the right column preserves each original prediction. Panel B compares six binary PLS-LDA score ranks and calls with development class counts. All ranks are explicitly not probabilities. This is an interface illustration, not external validation or representative case sampling.",
    width=6.7,
)
add_figure(doc, "Figure8_foundation_model_comparison.png", "Figure 8. Matched patient-level comparison of TITAN, Giga-SSL and Prov-GigaPath. Panel A reports the number of cancer–endpoint tasks meeting the prespecified internal screening threshold on the common cohort. Panel B compares the target-level effect statistic (Q² for continuous outcomes; 2×balanced accuracy−1 for binary outcomes) against TITAN. Panel C shows which representations retain screening-positive tasks; screen-negative tasks are omitted from this panel for readability. Identical patients, folds, seeds and tuning rules were used within every task. These are internally derived TCGA estimates, not evidence of external transportability.")

doc.add_heading("Discussion", level=1)
doc.add_paragraph(f"This study is best interpreted as a patient-level pathology foundation-model benchmark and reusable fitted-model resource, not as the first pan-cancer molecular-prediction study or a catalogue of unprecedented mutation targets. Selected immune programmes, mutations and higher-level genomic phenotypes showed held-out cross-validated statistical association, but conclusions were representation-dependent. On a rigorously matched cohort TITAN produced the broadest set of screening-positive tasks, yet Giga-SSL or Prov-GigaPath yielded higher effect statistics for hundreds of individual endpoints. Foundation-model selection should therefore be treated as part of model development and uncertainty, not as a neutral preprocessing choice. Here and throughout, operational ‘predictability’ neither establishes causal biological information nor demonstrates recovery or replacement of the originating assay.")
doc.add_paragraph("The three-way comparison is translationally relevant because an association that depends strongly on one representation is a weaker candidate for locked evaluation than one retained across distinct pretraining strategies. Cross-representation retention is not independent validation: all embeddings derive from TCGA slides, share the same outcomes and may encode the same cohort-specific artefacts. It nevertheless provides a practical robustness axis and a transparent basis for choosing which representation-specific fitted object to test next. The PathoFMPred interface exposes that choice explicitly rather than silently assuming that one foundation model is universally preferable.")
...
doc.add_paragraph("Using patient-level mean-pooled TITAN, Giga-SSL and Prov-GigaPath representations, this study shows that foundation-model choice materially affects internal molecular and derived immune-feature performance. TITAN was strongest in aggregate on the matched TCGA tasks, but no representation was uniformly best. PathoFMPred provides a model-aware research interface for locked follow-up without distributing patient-level training data. All results remain internally derived TCGA estimates; prediction of an inferred target is not evidence that a direct assay has been reproduced. The contribution is a transparent comparative computational-pathology benchmark and research-software framework—not external performance evidence or clinical validation—and independent testing remains essential.")
...
for h, text in [
    ...
    ("Availability of data and materials", "The analysis used the official TITAN TCGA feature artifact, official Giga-SSL TCGA embeddings and the seandavis/tcga_provgigapath_embeddings Hugging Face dataset. Conversion scripts, exact source locations, commits, access conditions and SHA-256 checksums are recorded in the companion repository [30]. Controlled-access whole-slide images are not redistributed. TCGA molecular data remain available from their cited sources. The separate PathoFMPred package and fitted research objects are maintained in a private access-controlled repository [32]; this manuscript does not claim that those artifacts are publicly downloadable."),
...
doc.add_paragraph(
    ...
    "Methods, Tables S1–S15, Figures S1–S4 and an inventory of the companion "
    "machine-readable result files."
)
doc.add_paragraph(
    "Additional file 2 (.pdf): COAD example A (TCGA-AA-A01F) PathoFMPred research-software output using TITAN features. "
...
doc.add_paragraph(
    "Additional file 3 (.pdf): COAD example B (TCGA-AA-3972) PathoFMPred research-software output using TITAN features. "
    "Post hoc supplementary interface illustration containing internally derived full-cohort TCGA estimates; not probability-calibrated or externally validated."
)
...
f"30. Pathology foundation-model patient-level benchmark repository. GitHub. {REPO}. Accessed 24 Aug 2026.",
...
f"32. PathoFMPred R package and fitted-model repository. GitHub. {MODEL_REPO}. Accessed 24 Aug 2026.",
...
"39. Lazard T, Lerousseau M, Decencière E, Walter T. Giga-SSL: self-supervised learning for gigapixel images. arXiv. 2022;arXiv:2212.03273. doi:10.48550/arXiv.2212.03273.",
...
doc.save(OUT / "manuscript_JTM_foundation_model_comparison.docx")
...
if pathology_qc_fields:
    ...
    sup.add_paragraph(
        ...
        f"{ival(pathology_qc.get('no_residual_tumour_mentions'))}. Complete slide-multiplicity and audit outputs are in "
        "pathology_qc_patient_multiplicity_audit.csv, pathology_qc_narrative_audit.csv and pathology_qc_maximum_slide_patient.csv."
    )
...
sup.add_heading("Table S10b. Screen-positive models below the original prespecified screening threshold under validation grouped by TCGA tissue-source-site code", 1)
...
sup.add_paragraph("TCGA tissue-source-site codes represented by fewer than 10 patients were excluded only from this dedicated confounding analysis. Macro balanced accuracy is the mean recall across codes; its chance reference is 1/k for k analysed codes. Good code prediction demonstrates that the representation carries information about TCGA tissue-source-site code, but the code is not an institution, scanner, laboratory or staining-batch identifier.")
add_table(
    sup,
...
sup.add_heading("Table S15. Matched foundation-model cohort and performance comparison", 1)
add_table(
    sup,
    ["Representation", "Slides", "Patients", "Dimensions", "Multi-slide patients", "Continuous positive", "Binary positive"],
    [(
        m, foundation_cohort_by_model[m]["source_slides"],
        foundation_cohort_by_model[m]["patients"],
        foundation_cohort_by_model[m]["dimensions"],
        foundation_cohort_by_model[m]["multi_slide_patients"],
        foundation_summary_by_key[(m, "continuous")]["screening_positive"],
        foundation_summary_by_key[(m, "binary")]["screening_positive"],
    ) for m in ("TITAN", "GigaSSL", "ProvGigaPath")],
    [4.0, 2.6, 2.6, 2.4, 3.2, 3.2, 3.0],
)
sup.add_paragraph("All performance comparisons used the same 8,241-patient intersection and identical outcome-labelled patient subsets, folds, seeds and tuning rules within each task. 'Positive' denotes crossing the prespecified internal screening-statistic threshold before representation-specific permutation testing; it is not evidence of external validity.")
add_portrait_section(sup)
...
responses = [
    ...
    ("Audit comment: the Introduction did not state the gap",
     "Addressed. The Background now states explicitly that breadth is not the gap after the large pan-cancer studies by Fu, Kather, Saldanha and especially Arslan. It defines the narrower gap as a reproducible patient-level benchmark using one fixed pretrained representation, deterministic slide aggregation, one nested validation framework, complete negative and ineligible reporting, target-level sensitivity to grouping by TCGA tissue-source-site code and compact fitted research models."),
    ...
    ("Move implementation detail out of the main Results",
     "Agreed. The TITANPred inventory, input schema, pooling behaviour, score-rank construction, checksums, class counts, reliability warnings, report-generation behaviour, synthetic smoke test, locked-target details and artifact metadata have been removed from the main Results. These materials remain in Supplementary Methods, Tables S6b–S6c and S13, the machine-readable registries and package documentation. The implementation-oriented 'External-validation readiness audit' subsection and the software-construction paragraph have been deleted. The main Results now contain one sentence stating the absence of external validation and a short 'Illustrative COAD profiles' section limited to the case-selection caveat and biological interpretation of Figure 7."
    ),
...
for source_name, output_name in (
    ("COAD_example_A.pdf", "Additional_file_2_COAD_example_A_PathoFMPred_report.pdf"),
    ("COAD_example_B.pdf", "Additional_file_3_COAD_example_B_PathoFMPred_report.pdf"),
):
    shutil.copyfile(ROOT / "results" / "reports" / source_name, OUT / output_name)

print(OUT / "manuscript_JTM_foundation_model_comparison.docx")
