...
doc.add_heading("Sensitivity to TCGA tissue-source-site-code grouping", level=2)
doc.add_paragraph(
    ...
    "colorectal APC results are not adequately summarised by the median and are prominently labelled as sensitive to TCGA tissue-source-site-code grouping in the registry and research-software output. "
)
doc.add_paragraph(
...
doc.add_paragraph(
    ...
    + ". ACC, CHOL, DLBC, MESO and UCS had fewer than two codes meeting the 10-patient requirement and were reported as ineligible. This result shows substantial TCGA tissue-source-site-code information in the fixed TITAN representation and strengthens the confounding concern; it does not identify a causal scanner, laboratory or staining effect."
)
add_figure(doc, "Figure6_site_grouped_sensitivity.png", "Figure 6. Sensitivity to TCGA tissue-source-site-code grouping as a principal result. Panel A compares all 323 random-fold estimates with estimates grouped by TCGA tissue-source-site code; Panel B shows the six largest attenuations; Panel C shows the 10 strongest chance-normalised within-cancer code-classification results. Larger typography and restricted ranked panels improve readability. This grouping remains internal to TCGA; the code is not an institutional, scanner or laboratory identifier, and the analysis is not external validation.")
...
if highlighted:
    ...
    for r in highlighted:
        ...
        provenance_metadata = (
            ...
            f'TCGA TSS codes: {r.get("site_grouped_n_sites")}; '
            f'delta: {fnum(r.get("site_performance_delta"))}; '
            f'status: {r.get("site_robustness_status")}; '
            f'inner TSS-code separation: {r.get("site_grouped_inner_site_separation")}; '
...
add_table(sup, ["Type", "Family", "Cancer", "Endpoint", "n", "TCGA TSS codes", "Primary screen metric", "TSS-code-grouped metric", "Delta"],
...
add_table(
    sup,
    ["Model", "Fold", "Test patients", "Test TSS codes", "Test positive/negative", "Training patients", "Training TSS codes", "Training positive/negative"],
    [(
...
add_table(
    sup,
    ["Cancer", "Eligible", "Analysed patients", "Analysed TSS codes", "Macro BA, mean (SD)", "Chance 1/k", "Chance-normalised BA", "Reason if ineligible"],
    [(
