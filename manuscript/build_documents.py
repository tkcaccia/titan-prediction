...
doc.add_paragraph("The primary question was cancer-specific: which individual mutations, derived inflammatory features, oncogenic pathways, MSI phenotypes, aneuploidy measures and fusions show held-out cross-validated statistical association with the TITAN representation within each cancer? Throughout this article, ‘predictability’ is shorthand only for that operational cross-validated association at the reported performance threshold. It does not denote causal biological information, a morphological mechanism, direct recovery of the source measurement or justification for replacing an assay. Molecular subtype association was not analysed. One-at-a-time PLS1 and PLS–LDA formed the prespecified reference screen because they preserve outcome-specific denominators and yield one independently usable model per endpoint. Joint PLS2 was evaluated secondarily only for coherent continuous inflammatory panels; its complete-case panel, shared latent structure and multiresponse estimand are not interchangeable with the endpoint-specific atlas.")
