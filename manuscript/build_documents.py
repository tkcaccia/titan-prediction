...
sup.add_paragraph("This matrix records the authors' current release policy and is not a legal determination. Upstream terms remain authoritative and must be checked at the version used. A GPL licence on original package code does not relicense fitted objects, upstream representations or source datasets. No durable reader-facing route to the controlled objects is presently promised. The intended public model-free shell supports registry inspection, schema validation, tests and synthetic fixtures but cannot perform endpoint inference without a separately supplied permitted object.")
add_table(
    sup,
...
responses = [
    ...
    ("4. Reproducibility materials must be deposited before submission",
     f"The analysis repository ({REPO}) remains public and contains the analysis plan, source URLs/checksums, software commit, eligibility tables, code, out-of-fold outputs, literature crosswalk, model registry and locked external-evaluation protocol. At the authors' request, the PathoFMPred repository ({MODEL_REPO}) is now private. The manuscript no longer claims that the package or its {registry_object_total} fitted objects are publicly downloadable. A public model release, if permitted, and a persistent DOI remain future release decisions; reviewers can be granted controlled access when authorised."),
    ...
    ("9. The software-resource claim needs a clearer access and licensing model",
     f"Agreed. We now describe PathoFMPred as a reproducible analysis interface and model registry with partial, access-controlled internal reference objects, not a reusable fitted-model resource. The public analysis source is GPL-3.0-or-later; PathoFMPred source is GPL-3.0 but private during review; fitted coefficient objects have no public redistribution licence and remain access-controlled; and all upstream representation and dataset terms remain authoritative. During review, source, the complete registry, representation-specific schemas, tests, synthetic fixtures and hashes can be inspected, while object-level inspection requires case-specific approval through a confidential route. No durable reader-facing object service currently exists. At publication, the intended public artifact is a model-free package shell with registry, schemas, tests and a synthetic example; it cannot perform endpoint inference without a separately supplied permitted object, and no fitted object is presently claimed as publicly downloadable. The controlled registry contains {registry_object_total} genuinely representation-specific coefficient objects: {inventory_by_representation['TITAN']} TITAN, {inventory_by_representation['GigaSSL']} Giga-SSL and {inventory_by_representation['ProvGigaPath']} Prov-GigaPath. These counts are not matched crossing counts: the two alternative inventories use the identical shared TITAN-qualified matched-eligible target universe. Selecting foundation_model changes both schema and coefficients when an object exists, but the registry does not cover every representation-specific crossing. TITAN-derived objects remain restricted pending written upstream clarification; Giga-SSL and Prov-GigaPath terms are recorded separately. This is explicitly presented as an access-policy statement, not a legal determination."),
...
responses.append((
    ...
    f"Agreed. The identical 297-object counts were not representation-specific crossing inventories. Both Giga-SSL and Prov-GigaPath were fitted on the same 297 targets inherited from the permutation/FDR-qualified TITAN catalogue that remained eligible in the 8,241-patient matched cohort: 198 continuous and 99 binary targets. This shared-universe design avoided selecting objects after inspecting each alternative representation's performance, but it meant that object presence and matched effect-threshold crossing were different facts. Giga-SSL objects cover {inventory_reconciliation_value('GigaSSL', 'continuous', 'representation_crossings_with_controlled_object')}/105 continuous and {inventory_reconciliation_value('GigaSSL', 'binary', 'representation_crossings_with_controlled_object')}/63 binary crossings; 12 continuous and 13 binary Giga-SSL crossings have no controlled object. Prov-GigaPath objects cover {inventory_reconciliation_value('ProvGigaPath', 'continuous', 'representation_crossings_with_controlled_object')}/130 and {inventory_reconciliation_value('ProvGigaPath', 'binary', 'representation_crossings_with_controlled_object')}/76; 18 and 15 crossings, respectively, have no object. Conversely, 105 continuous and 49 binary Giga-SSL objects and 86 continuous and 38 binary Prov-GigaPath objects did not cross for that representation. The TITAN registry separately contains all 323 candidates from the larger TITAN-only permutation/FDR-qualified layer and is therefore also not numerically identical to its common-cohort matched crossings. We added a target-level executable audit and two machine-readable files, fitted_model_inventory_reconciliation.csv and fitted_model_target_universe_audit.csv. Supplementary Table S6d now shows crossings, objects, below-threshold objects, uncovered crossings and coverage percentages side by side. Every registry row now records the controlled-object target universe, matched representation-specific crossing status, coverage note and the fact that the inventory is incomplete. compare_pathofm_models() was corrected so that it never infers consensus from object count: it reports controlled-object availability, matched-threshold status, selection basis and partial resource scope separately, and explains that absence of an object does not imply an untested or negative atlas task. The manuscript no longer describes PathoFMPred as a complete operationalization or publicly functional fitted-prediction resource. It is now a reproducible analysis interface and model registry with partial, access-controlled internal reference objects. Because no durable reader-facing controlled-object route currently exists, the intended public model-free shell is stated to support registry inspection, schema validation, tests and synthetic fixtures but not endpoint inference without a separately supplied permitted object. The final archived release, immutable synchronized commit and DOI remain explicit author-dependent blockers before acceptance and were not invented for this working tree."
))
responses.append((
