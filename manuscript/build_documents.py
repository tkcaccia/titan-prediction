...
if abstract_continuous and abstract_binary:
    abstract_metric_text = (
        ...
        f'(Q² {fnum(abstract_continuous.get("q2"))}, selection-conditioned 95% patient-resampling interval '
...
if highlighted:
    ...
    doc.add_paragraph("Table 3. Highlighted repeated nested-cross-validation performance, binary reliability and tissue-source-site-grouped internal sensitivity. SC interval denotes the 95% selection-conditioned patient-resampling interval for repeated out-of-fold predictions: it resamples patients from five fixed prediction sets without repeating screening, fold generation, tuning or model fitting. Panel A reports discrimination/error metrics and grouped sensitivity. Panel B adds class-size and prevalence-dependent binary metrics. These intervals, grouped estimates and predictive values are internal to TCGA, not generalisation confidence intervals or external-validation results.", style="Caption")
    ...
    add_table(doc, ["Cancer–endpoint", "Type", "Q² or Se/Sp (95% SC interval)", "RMSE or BA (95% SC interval)", "Spearman or AUROC (95% SC interval)", "TSS-grouped metric/status"], performance_rows)
    doc.add_paragraph("Panel B. Binary class-size and prevalence-dependent reliability metrics.", style="Caption")
    add_table(
        doc,
        ["Cancer–endpoint", "+/−", "PR-AUC (95% SC interval)", "PPV at TCGA prevalence (95% SC interval)", "NPV at TCGA prevalence (95% SC interval)", "Evidence/default"],
        [(
...
doc.add_paragraph(
    ...
    f"The symmetric binary results do not support a general advantage for either algorithm: the AUROC interval excluded zero in favour of ridge for {binary_primary_ridge_labels}, while no symmetrically thresholded balanced-accuracy interval excluded zero. For continuous Q², ridge-favouring intervals were observed for {continuous_primary_ridge_labels}. These are conditional comparisons of highlighted endpoints, not multiplicity-controlled algorithm discoveries. Because the benchmark was restricted to PLS-highlighted results, the prespecified atlas was retained and the comparison is reported as selection-conditioned sensitivity evidence rather than a basis for post-screen method switching."
)
...
doc.add_paragraph(f"The ridge benchmark prevents interpreting that design choice as proof of PLS superiority. The corrected binary comparison removed the previous decision-rule asymmetry: both methods used identical outer and inner folds and selected operating thresholds from inner out-of-fold scores using the same balanced-accuracy criterion. Threshold-independent AUROC was primary for this method comparison. The median ridge-minus-PLS AUROC difference was {fnum(binary_baseline_summary.get('median_delta_ridge_minus_pls'))}, with {sum(r.get('outcome_type') == 'binary' for r in ridge_better)} endpoints favouring ridge and {sum(r.get('outcome_type') == 'binary' for r in baseline_uncertain)} uncertain; the median balanced-accuracy difference under symmetric thresholding was {fnum(binary_baseline_summary.get('median_secondary_delta_ridge_minus_pls'))}, with {len(binary_secondary_ridge)} favouring ridge, {len(binary_secondary_pls)} favouring PLS and {len(binary_secondary_uncertain)} uncertain. The continuous median Q² difference was {fnum(continuous_baseline_summary.get('median_delta_ridge_minus_pls'))}; {sum(r.get('outcome_type') == 'continuous' for r in ridge_better)} intervals excluded zero in favour of ridge. These paired patient-resampling intervals condition on the PLS screen and five existing matched prediction sets; they do not repeat endpoint selection, generate new partitions or refit models within bootstrap samples. Both approaches yield compact exportable parameters, so portability is not unique to PLS. PLS remains the prespecified atlas method because the benchmark was performed only on PLS-highlighted models and switching methods after that selection would add another optimistic selection layer. Future external validation can compare locked PLS and ridge candidates prospectively.")
...
if highlighted:
    ...
    sup.add_paragraph("SC interval denotes the 95% selection-conditioned patient-resampling interval for repeated out-of-fold predictions. Patients are resampled from five fixed held-out prediction sets; screening, new fold generation, tuning and model fitting are not repeated within bootstrap replicates.")
    performance_rows = []
    deployment_rows = []
...
sup.add_paragraph("The benchmark is conditional on selection of PLS screen-positive highlighted models. Both algorithms used identical patients, outer folds and inner folds; binary models also used the same inner out-of-fold balanced-accuracy threshold-selection rule. For repeat r, d_r=M_r(ridge)−M_r(PLS), with Δ=(1/5)Σ_r d_r. In each of 2,000 paired patient-resampling replicates, patients were sampled with replacement and all five predictions from both algorithms were retained; d_r and then Δ were recalculated. The 2.5th and 97.5th percentiles form the reported selection-conditioned paired interval. No new partitions were generated and neither algorithm was refitted inside a bootstrap replicate. Screening/highlighting uncertainty and external transportability are not represented. Panel A reports threshold-independent AUROC for binary models and Q² for continuous models. Panel B reports symmetrically thresholded balanced accuracy for binary models and Spearman correlation for continuous models. Positive differences favour ridge.")
sup.add_paragraph("Panel A. Primary algorithm-comparison metric.", style="Caption")
add_table(sup, ["Type", "Family", "Cancer", "Endpoint", "Metric", "PLS", "Ridge", "Δ Ridge−PLS", "95% paired patient interval", "Result"],
