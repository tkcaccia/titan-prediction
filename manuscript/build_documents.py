...
add_labelled(doc, "Results.", f"Within-cancer screening identified {len(supported_c):,} continuous and {len(supported_b):,} binary higher- or moderate-effect candidates. Highlighted repeated-CV results included {abstract_metric_text}. The literature audit found prior statistical support for {len(prior_supported_mutations)}/{len(mutation_literature_audit)} screen-positive cancer–gene pairs. Under the multiplicity sensitivity, {global_abstract_text}; its attainable q-values in large families were limited by the 0.001 empirical-p resolution. Tissue-source-site grouping reduced {ival(site_combined.get('below_threshold_models'))}/{ival(site_combined.get('screen_positive_models'))} candidates ({fnum(site_combined.get('below_threshold_percent'), 1)}%) below the original effect threshold. Within-cancer TITAN representations predicted tissue-source site in {len(site_predictability_eligible)}/32 cancers, with median multiclass macro balanced accuracy {fnum(median([r.get('macro_balanced_accuracy_mean') for r in site_predictability_eligible]))}. In a selection-conditioned 24-model benchmark, median ridge-minus-PLS differences were {fnum(binary_baseline_summary.get('median_delta_ridge_minus_pls'))} AUROC for binary models and {fnum(continuous_baseline_summary.get('median_delta_ridge_minus_pls'))} Q² for continuous models; the median binary balanced-accuracy difference under identical inner-CV threshold selection was {fnum(binary_baseline_summary.get('median_secondary_delta_ridge_minus_pls'))}.")
...
if highlighted:
    ...
    doc.add_paragraph("Table 3. Highlighted repeated nested-cross-validation performance with 95% patient-cluster bootstrap intervals and tissue-source-site-grouped internal sensitivity. Site-grouped values are internal TCGA estimates, not external-validation results.", style="Caption")
    performance_rows = []
    for r in h_cont:
...
doc.add_paragraph(
    ...
    "repeated-CV performance, the tissue-source-site-grouped metric and any limited-class-size or site-sensitivity warning beside the call. A single-sample research-software output renders all available continuous endpoints "
