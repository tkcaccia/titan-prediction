...
target_hashes <- c(
  nonmutation = digest::digest(
    file = "data/processed/binary_targets_nonmutation.rds", algo = "sha256"
  ),
  mutation = if (file.exists(mutation_path)) {
    digest::digest(file = mutation_path, algo = "sha256")
  } else NA_character_
)
analysis_fingerprint <- digest::digest(list(
  checkpoint_schema = 2L,
  script_sha256 = digest::digest(file = "R/05_screen_binary.R", algo = "sha256"),
...
if (anyDuplicated(expected_ids)) stop("Binary jobs produced duplicate checkpoint IDs")
expected_checkpoints <- file.path(checkpoint_dir, paste0(expected_ids, ".rds"))
if (!all(file.exists(expected_checkpoints))) stop("A binary checkpoint is missing")
...
}, logical(1)))) stop("A binary checkpoint has a stale analysis fingerprint")
