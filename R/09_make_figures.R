...
ggsave("figures/Figure2_continuous_atlas.png", p2, width = 10.8, height = 6.8,
       dpi = 320, bg = "white")
...
largest_declines <- site[order(delta)][seq_len(min(6L, .N))]
...
p6b <- ggplot(largest_declines, aes(y = label)) +
  ...
  labs(
    ...
    subtitle = "Largest losses hidden by the median",
...
site_prediction <- site_prediction[order(-normalized_macro_balanced_accuracy)][seq_len(min(10L, .N))]
...
p6 <- (p6a | p6b) / p6c +
  plot_annotation(
    ...
    theme = theme(
      ...
      plot.caption = element_text(color = muted, hjust = 0, size = 10)
    )
  )
